...
def coolpush(token, msg):
    return json.loads(
        requests.post("https://push.xuthus.cc/send/" + token, data=msg).content
    )["message"]
...
class Oshwhub:
    ...
    def auto_sign(self):
        ...
        if self.sign_flag:
            ...
            oshw_cookies["oshwhub_session"] = cookies2dict(
                oshw_res.headers["Set-Cookie"]
            )["oshwhub_session"]
            # print(oshw_cookies)
        ...
        if self.seven_reward_flag:
            ...
            coupon_uuid = json.loads(oshw_res.content)["result"]["sevenDay"][
                "coupon_uuid"
            ]
            ...
            if not json.loads(oshw_res.content)["code"]:
                self.seven_reward_Statistics = (
                    "七天奖励领取成功, 奖品为: " + json.loads(oshw_res.content)["result"]["info"]
                )
            else:
...
if __name__ == "__main__":
    try:
        ...
        for key in users:
            ...
            if (
                my_user.sign_flag
                or my_user.three_reward_flag
                or my_user.seven_reward_flag
            ):
                if len(coolpush_token) == 32:
                    logger.info(
                        "推送结果: "
                        + coolpush(coolpush_token, coolpush_payload.encode("UTF-8"))
                    )
                elif len(pushplus_token) == 32:
                    logger.info(
                        "推送结果: "
                        + json.loads(
                            requests.post(url_pushplus, data=push_payload).content
                        )["data"]
                    )
                elif agentid != 0:
                    logger.info(
                        "推送结果: "
                        + json.loads(
                            weixinpush(corpid, corpsecret, agentid, coolpush_payload)
                        )["errmsg"]
                    )
                else:
